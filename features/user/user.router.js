...
router.post(
  '/get-grantaccess-data',
  authMiddleware,
  controller.getGrantAccessData
);
router.post(
  '/request-grant/:mechanicId/overtime',
  authMiddleware,
  controller.requestGrant
);
router.post('/request-service', controller.requestService);

// ── Document signing ──────────────────────────────────────────────────────────
router.post(
  '/six-digit-auth/verify',
  authMiddleware,
  controller.verifyDocAuthUser
);
...
router.post(
  '/doc-oauth-accounts-sign-key',
  authMiddleware,
  controller.getSignAccountsKey
);
router.post(
  '/doc-oauth-manager-sign-key',
  authMiddleware,
  controller.getSignManagerKey
);
router.post(
  '/doc-oauth-authorized-sign-key',
  authMiddleware,
  controller.getSignAuthorizedKey
);
router.post('/doc-oauth-seal-sign-key', authMiddleware, controller.getSealKey);
router.post(
  '/activate-signature',
  authMiddleware,
  controller.activateSignature
);
router.post(
  '/verify-device-trust',
  authMiddleware,
  controller.verifyDeviceTrust
);

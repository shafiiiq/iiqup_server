...
const { GMAIL_SCOPES } = require('../../shared/constants/email.constant');
const { loadImageAsBase64, getMimeType } = require('../../shared/helpers/email.helper');
...
class OAuth2GmailClient {
  ...
  /**
   * Initialise the OAuth2 client and Gmail API instance.
   * Returns true when ready, false when a refresh token is still needed.
   */
  async initialize() {
  ...
  /**
   * Generate the one-time authorization URL used to obtain a refresh token.
   */
  getAuthUrl() {
  ...
  /**
   * Exchange a one-time authorization code for OAuth2 tokens.
   */
  ...
  /**
   * Fetch the default Gmail send-as signature for the authenticated account.
   */
  async getSignature() {
    try {
  ...
  /**
   * Resolve a single attachment entry into { fileContent, filename, mimeType }.
   * Returns null and emits a warning when the attachment cannot be resolved.
   */
  ...
  /**
   * Build a raw MIME message string ready for base64url encoding.
   */
...
/**
 * Build the HTML body for a Backcharge notification email.
 */
...
/**
 * Send a Backcharge notification email with optional PDF attachments.
 */
...
/**
 * Generate the one-time OAuth2 authorization URL (setup only).
 */
...
/**
 * Exchange a one-time authorization code for OAuth2 tokens (setup only).
 */

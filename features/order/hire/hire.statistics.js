...
const getAllTimeTotals = async () => {
  const [result] = await HireOrder.aggregate([
...
const getYearRangeFromData = async () => {
  const oldest = await HireOrder.findOne({}).sort({ createdAt: 1 }).select('createdAt').lean()
...
const getSeriesByYear = async () => {
  ...
  const grouped = await HireOrder.aggregate([
    {
      $group: {
        _id: { $year: '$createdAt' },

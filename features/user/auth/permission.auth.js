...
const mechanicServices = require('../../mechanic/mechanic.service');
const { formatDate, convertToAMPM } = require('../user.helper');
  ...
  try {
    ...
    if (purpose === 'overtime')
      receiver = await User.findOne({
        uniqueCode: process.env.OVERTIME_AUTHORISE_CODE,
      });
    if (!receiver)
      return {
        status: 404,
    ...
    try {
      ...
      const notificationMessage =
        data.times?.[0]?.in && data.times?.[0]?.out
...
const approveRequest = async (uniqueCode, dataId, purpose) => {
  try {
    ...
    const requestIndex = user.grantAccess.findIndex(
      (r) => r._id.toString() === dataId && r.purpose === purpose
    );
    if (requestIndex === -1)
    ...
    try {
      const response = await mechanicServices.addOvertime(
        mechanicId,
        dataToSend
      );
      ...
      try {
        ...
        if (updatedUser) {
          updatedUser.grantAccess.splice(requestIndex, 1);
      ...
      try {
        ...
        const notificationMessage =
          dataToSend.times?.[0]?.in && dataToSend.times?.[0]?.out
        ...
        const notification = await createNotification({
          title: 'Mechanic overtime accepted',
          description: notificationMessage,
          priority: 'high',
          type: 'normal',
        });

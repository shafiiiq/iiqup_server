 ...
 * @returns {Promise}
 */
const activateSignatureAccess = async (
  userId,
  activationKey,
  signType,
  deviceInfo
  ...
  try {
    ...
    const signActivation = user.signatureActivation.find(
      (s) => s.signType === signType
    );
    if (!signActivation)
      throw {
        status: 404,
        message: 'Signature type not configured for this user',
      };

    const isKeyValid = await bcrypt.compare(
      activationKey,
      signActivation.activationKey
    );
    if (!isKeyValid) throw { status: 401, message: 'Invalid activation key' };

    const { deviceFingerprint, ipAddress, location, userAgent, browserInfo } =
      deviceInfo;
    if (
      !deviceFingerprint ||
      !ipAddress ||
    ...
    if (existingDeviceIndex !== -1) {
      ...
      signActivation.trustedDevices.push({
        uniqueCode: encFingerprint.encryptedData,
        uniqueCodeIv: encFingerprint.iv,
        ipAddress: encIp.encryptedData,
        ipAddressIv: encIp.iv,
        location: encLocation.encryptedData,
        locationIv: encLocation.iv,
        userAgent: encUserAgent.encryptedData,
        userAgentIv: encUserAgent.iv,
        browserInfo: encBrowser.encryptedData,
        browserInfoIv: encBrowser.iv,
...
const verifyTrustedDevice = async (userId, signType, deviceInfo) => {
  try {
    ...
    if (!signActivation || !signActivation.isActivated)
      return { status: 200, data: { isActivated: false, isTrusted: false } };
    ...
    if (!trustedDevice)
      return { status: 401, data: { isActivated: false, isTrusted: false } };
...
const getSignKey = async (userId, signType, envKey, deviceInfo) => {
  ...
  return {
    status: 200,
    data: { sign_key: process.env[envKey], expiresIn: 30 },
  };
};

const getAuthSignKey = async (password) => {
  const { verifyDocAuthUserCreds } = require('./user.service');
...
const getWmAuthSignKey = (userId, deviceInfo) =>
  getSignKey(userId, 'wm', 'WM_SIGN_KEY', deviceInfo);
const getPmAuthSignKey = (userId, deviceInfo) =>
  getSignKey(userId, 'pm', 'PM_SIGN_KEY', deviceInfo);
const getAccountsAuthSignKey = (userId, deviceInfo) =>
  getSignKey(userId, 'accounts', 'ACCOUNTS_SIGN_KEY', deviceInfo);
const getManagerAuthSignKey = (userId, deviceInfo) =>
  getSignKey(userId, 'manager', 'MANAGER_SIGN_KEY', deviceInfo);
const getAuthorizedAuthSignKey = async (userId, deviceInfo, authRole) => {
  ...
  const signKey =
    authRole === 'MANAGING_DIRECTOR' ? baseKey.replace(/ceo/gi, 'md') : baseKey;

  return { status: 200, data: { sign_key: signKey, expiresIn: 30 } };
};
const getAuthSealKey = (userId, deviceInfo) =>
  getSignKey(userId, 'seal', 'SEAL_KEY', deviceInfo);

...
const startOfWeek = (date) => { const d = startOfDay(date); d.setDate(d.getDate() - d.getDay()); return d }
const endOfWeek = (date) => { const d = startOfWeek(date); d.setDate(d.getDate() + 6); return endOfDay(d) }
const startOfMonth = (date) => { const d = new Date(date.getFullYear(), date.getMonth(), 1); d.setHours(0, 0, 0, 0); return d }
const endOfMonth = (date) => { const d = new Date(date.getFullYear(), date.getMonth() + 1, 0); return endOfDay(d) }
const startOfYear = (date) => { const d = new Date(date.getFullYear(), 0, 1); d.setHours(0, 0, 0, 0); return d }
const endOfYear = (date) => { const d = new Date(date.getFullYear(), 11, 31); return endOfDay(d) }
...
const sumDocsIntoBuckets = (docs, config) => {
  ...
  for (const doc of docs) {
    ...
    buckets[index].total += doc.costSummary?.totalCost || 0
    buckets[index].deduction += doc.costSummary?.approvedDeduction || 0
...
const fetchDocsInRange = async (rangeStart, rangeEnd) =>
  Backcharge.find(
    ...
    { createdAt: 1, costSummary: 1 }
  ).lean()

const sumTotals = (docs) =>
  docs.reduce(
    (acc, doc) => {
      acc.total += doc.costSummary?.totalCost || 0
      acc.deduction += doc.costSummary?.approvedDeduction || 0
      acc.count += 1
      return acc
    },
    { total: 0, deduction: 0, count: 0 }
  )

const getAllTimeTotals = async () => {
  const [result] = await Backcharge.aggregate([
    {
      $group: {
        _id: null,
        total: { $sum: '$costSummary.totalCost' },
        deduction: { $sum: '$costSummary.approvedDeduction' },
        count: { $sum: 1 },
      },
    },
  ])
  ...
  return {
    ...
    averageDeduction: count > 0 ? deduction / count : 0,
  }
}

const getYearRangeFromData = async () => {
  const oldest = await Backcharge.findOne({}).sort({ createdAt: 1 }).select('createdAt').lean()
...
const getSeriesByYear = async () => {
  ...
  const grouped = await Backcharge.aggregate([
    {
      $group: {
        _id: { $year: '$createdAt' },
        total: { $sum: '$costSummary.totalCost' },
  ...
  const totalsByYear = new Map(
    grouped.map((row) => [row._id, { total: row.total || 0, deduction: row.deduction || 0, count: row.count || 0 }])
  )

  const buckets = years.map((year) => {
    const entry = totalsByYear.get(year) || { total: 0, deduction: 0, count: 0 }
    return { label: String(year), total: entry.total, deduction: entry.deduction, count: entry.count }
  })
...
const getSummary = async (granularity) => {
  ...
  if (granularity === 'allYears') {
    ...
    const current = series.buckets[series.buckets.length - 1] || { total: 0, deduction: 0, count: 0 }
    const previous = series.buckets[series.buckets.length - 2] || { total: 0, deduction: 0, count: 0 }
    const growthPercent =
    ...
    return {
      ...
      totalDeduction: current.deduction,
      ...
      averageDeduction: current.count > 0 ? current.deduction / current.count : 0,
      allTimeTotalCost: allTime.totalCost,
      allTimeTotalDeduction: allTime.totalDeduction,
      allTimeTotalCount: allTime.totalCount,
      allTimeAverageCost: allTime.averageCost,
      allTimeAverageDeduction: allTime.averageDeduction,
    }
  }
...
const getGrowth = async (granularity) => {
  ...
  const buckets = series.buckets.map((bucket) => {
    ...
    cumulativeDeduction += bucket.deduction
    return { ...bucket, cumulativeTotal, cumulativeDeduction }
  })

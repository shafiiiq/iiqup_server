...
const authorizedSignatorySchema = signatureSchema.clone().add({
...
const backchargeSchema = new mongoose.Schema(
  {
    ...
    signatures: {
      workshopManager: { type: signatureSchema, default: () => ({}) },
      purchaseManager: { type: signatureSchema, default: () => ({}) },
      operationsManager: { type: signatureSchema, default: () => ({}) },
      authorizedSignatory: { type: authorizedSignatorySchema, default: () => ({}) },
    },
...
const buildCombinedText = (field) => {
  if (!field?.lines?.length) return
...
backchargeSchema.pre('save', function (next) {
  buildCombinedText(this.scopeOfWork)
  buildCombinedText(this.workshopComments)
  next()
})

...
fuelTransactionSchema.statics.findByStation = function (
  stationName,
  startDate,
  endDate
) {
...
fuelTransactionSchema.statics.getMonthlyCustomerSummary = function (
  customerNumber,
  year,
  month
) {
...
fuelTransactionSchema.pre('save', function (next) {
  ...
  if (this.financialAccountName)
    this.financialAccountName = this.financialAccountName
      .toString()
      .trim()
      .toUpperCase();
  if (this.productName)

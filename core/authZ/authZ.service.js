...
const { verifyDocAuthUserCreds } = require('#features/user/staff/staff.service');
...
const decryptAndVerifyDeviceData = (encryptedData, iv, originalData) => {
  try {
    ...
    logger.error('[authZ.service] decryptAndVerifyDeviceData', error.message);
    return false;
  }
};
...
const activateSignatureAccess = async (userId, activationKey, signType, deviceInfo) => {
  try {
    ...
    const existingDeviceIndex = signActivation.trustedDevices.findIndex((d) => {
      ...
      return (
        decryptAndVerifyDeviceData(d.uniqueCode, d.uniqueCodeIv, deviceFingerprint) &&
    ...
    logger.error('[authZ.service] activateSignatureAccess', error);
    throw error;
  }
};
...
const verifyTrustedDevice = async (userId, signType, deviceInfo) => {
  try {
    ...
    logger.error('[authZ.service] verifyTrustedDevice', error);
    throw error;
  }
};
...
const getSignKey = async (userId, signType, envKey, deviceInfo) => {
  ...
  if (!result.data.isTrusted) {
    throw { status: 403, message: 'Device not trusted. Please activate signature access first.' };
  }
...
const getAuthSignKey = async (password) => {
  ...
  if (response.status !== 200) {
    return { status: 500, message: 'Failed to fetch doc sign key', error: response.message };
  }
...
const getAuthorizedAuthSignKey = async (userId, deviceInfo, authRole) => {
  ...
  const signKey = authRole === 'MANAGING_DIRECTOR' ? baseKey.replace(/ceo/gi, 'md') : baseKey;

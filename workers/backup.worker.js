...
//       logger.info(`[backup.worker] Deleted old backup: ${name}`);
//     } catch (err) {
//       logger.error(`[backup.worker] Could not delete ${name}:`, err.message);
//     }
//   });
// }
...
//   if (isRunning) {
//     logger.info('[backup.worker] Already running — skipped.');
//     return;
//   }

//   if (!MONGO_URI) {
//     logger.error('[backup.worker] MONGO_URI not set — skipping backup.');
//     return;
//   }
...
//   logger.info('[backup.worker] Starting backup...');
...
//     logger.info(
//       `[backup.worker] Database: ${dbName} | Collections: ${collections.length}`
//     );

//     if (collections.length === 0) {
//       logger.info('[backup.worker] No collections found — skipping.');
//       return;
//     }
...
//         logger.info(
//           `[backup.worker] ${colName}: ${docs.length} docs (${formatBytes(size)})`
//         );
...
//       } catch (err) {
//         logger.error(
//           `[backup.worker] Failed to back up "${colName}":`,
//           err.message
//         );
...
//     logger.info(
//       `[backup.worker] Backup complete — ${manifest.totalDocuments} docs | Stored: ${stored.length}/${MAX_BACKUPS} | Latest: ${timestamp}`
//     );
//   } catch (err) {
...
// logger.info('[backup.worker] Initialized — runs every 30 minutes.');
